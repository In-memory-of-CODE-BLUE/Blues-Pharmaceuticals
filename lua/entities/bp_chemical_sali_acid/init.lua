...
function ENT:Initialize()
	self:SetModel("models/blues_pharm/jar_2.mdl")
	...
	self:SetLiquidAmount(750)

	--Stores who currently has this jar selected
	self.playerWhoSelected = nil

	self:SetSubMaterial(0, "blues_pharm/jar_2_glass_sali_acid")
	self:SetSubMaterial(2, "blues_pharm/jar_2_label_sali_acid")
...
function ENT:Use(act, caller)
	...
	if caller:KeyDown(IN_SPEED) and self.angleResetCooldown < CurTime() then
		self:SetAngles(Angle(0,caller:GetAngles().y + 90,0))
